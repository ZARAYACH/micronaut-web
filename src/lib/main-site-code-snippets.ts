...
export async function renderMainSiteCodeSnippets(html: string) {
  ...
  for (const match of codeBlocks) {
    ...
    if (/\bshiki\b/.test(preAttributes) || /\bdocs-code-block\b/.test(preAttributes)) {
      result += match[0];
    ...
    const language = codeLanguageFromFenceInfo(codeLanguage(`${preAttributes} ${codeAttributes}`), source, "Main-site Markdown code block");
    result += await renderStaticCodeSnippetCard(`main-site-code-snippet-${snippetIndex}`, [{ language, source }]);
    position = (match.index ?? 0) + match[0].length;
    snippetIndex += 1;
  }
...
async function renderStaticCodeSnippetCard(snippetId: string, samples: CodeSample[]) {
...
async function renderSnippetPanel({
  ...
  const highlighted = await highlightCodeSnippetHtml(sample.source, highlighterLanguageFor(language));
...
function renderTemplate(source: string, replacements: Record<string, string>) {
  return source.replace(/{{(\w+)}}/g, (_match, key: string) => replacements[key] ?? "");
}

function codeLanguage(attributes: string) {
  const match = attributes.match(/\blanguage-([A-Za-z0-9_+-]+)/)
